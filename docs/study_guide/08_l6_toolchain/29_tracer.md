...
## 📚 학술적 배경: Symbolic tracing은 "compiler frontend"의 ML 버전

### 1. AST traversal — Dragon Book Ch.2

> Aho et al. — *Compilers* (2판), Ch.2 "Simple Compiler", §2.5 "Syntax-Directed Translation".

전통적 컴파일러: 소스 코드 → **lexer/parser** → AST → traversal → IR. 이 과정이 PyTorch에서는 다르게 일어남:
- PyTorch model은 이미 Python 객체 (nn.Module의 forward)
- → AST를 직접 만들 필요 없이 **Python 인터프리터를 hook**해서 호출 트레이스를 잡음
- 그것이 `torch.fx.symbolic_trace`. **fx.Proxy**가 가짜 텐서 역할 → forward()를 실행하면 연산 호출 사슬이 graph로 기록됨

이 tracer.py가 그 fx.Graph를 traversal하면서 `OpNode` 생성. Dragon Book의 syntax-directed translation을 ML 모델 객체에 적용한 셈.

### 2. Static (FX) vs Dynamic (TorchScript) tracing

PyTorch는 두 frontend가 있습니다:

| 방식 | 동작 | 한계 |
|---|---|---|
| **torch.fx (이 프로젝트)** | Python 함수 그대로 실행하면서 fx.Proxy로 graph 캡처 | 데이터 의존 제어 흐름(if x.sum() > 0)은 unwrap 필요 |
| torch.jit.trace | 한 sample 통과시켜 graph 기록 | 같은 한계 — 분기 잃음 |
| torch.jit.script | Python AST 직접 파싱 → TorchScript IR | 일부 Python feature 미지원 |

이 프로젝트는 **torch.fx** 선택. 이유:
- 산업계 inference engine (TensorRT, ONNX Runtime)도 fx 기반 frontend
- nn.Module 단위 hook이 정확 (call_module → 모듈 인스턴스 그대로 attrs로 가져옴)
- shape inference가 forward() 직접 실행이라 정확

📖 참고: [PyTorch FX docs](https://pytorch.org/docs/stable/fx.html), [PyTorch FX 논문 (Reed et al. MLSys'22)](https://arxiv.org/abs/2112.08429), Cooper & Torczon Ch.4 "Parsing" (Phase 3 자료).

### 3. Frontend의 책임 = "abstract gap의 첫 닫음"

ML 컴파일러 stack에서 frontend는 가장 **가변적**:
- ONNX, TF SavedModel, PyTorch nn.Module, JAX traced — 모두 다른 representation
- Backend(NPU bytecode)는 거의 변하지 않음
- → frontend가 "M source → 1 IR"의 M쪽을 흡수하는 첫 방어선

이 프로젝트의 IR이 **순수 Python**이라 하나의 frontend(이 파일)만 있으면 됨. 만약 ONNX support 추가하면 `onnx_tracer.py` 추가 — IR은 그대로. **MLIR 역시 같은 구조** (multiple dialects → 공통 backend lowering).
