...
module fpga_pynq_top #(
    ...
    parameter AXI_DATA_WIDTH  = 256,   // 16 × 16-bit FP16
    ...
    logic                      drain_valid;
    logic [15:0][511:0]        psum_capture_buf;
    logic [4:0]                drain_rx_cnt;
    logic                      drain_busy;
    logic [15:0][31:0]         psum_captured;   // Muxed output for the selected row
    ...
    // =========================================================================
    // Drain Capture Logic (AXIS Stream)
    // =========================================================================
    // 0x20 write -> mmio_drain_start -> receive 16 beats into buffer -> valid
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            mmio_drain_start <= 1'b0; // Pulse

            if (drain_cmd_we && !drain_busy) begin // Start sequence
                drain_busy       <= 1'b1;
                drain_valid      <= 1'b0;
                drain_rx_cnt     <= '0;
                mmio_drain_start <= 1'b1;
                mmio_drain_len   <= 9'd16; // Fixed 16 rows for basic test
                psum_tready      <= 1'b1;
            end else if (drain_busy) begin
                if (psum_tvalid && psum_tready) begin
                    psum_capture_buf[drain_rx_cnt[3:0]] <= psum_tdata;
                    drain_rx_cnt <= drain_rx_cnt + 1;
                    
                    if (drain_rx_cnt == 5'd15) begin
                        drain_busy  <= 1'b0;
                        drain_valid <= 1'b1;
                        psum_tready <= 1'b0;
                    end
    ...
    generate
        for (i = 0; i < 16; i++) begin : gen_mux
            assign psum_captured[i] = psum_capture_buf[drain_row_reg[3:0]][(i*32) +: 32];
        end
    endgenerate
