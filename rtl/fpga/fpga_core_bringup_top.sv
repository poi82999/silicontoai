...
module fpga_core_bringup_top (
    ...
    // FP32(120.0) = 0x42F00000 (sum of lane[0..15] * 1.0 = 120.0)
    ...
    logic [ROWS-1:0][15:0] core_activations;   // FP16
    logic [ROWS-1:0][15:0] core_weights;        // FP16
        ...
        case (state)
            ST_PRELOAD: begin
                for (int lane = 0; lane < ROWS; lane++) begin
                    core_weights[lane] = 16'h3C00; // FP16(1.0)
                end
            ...
            ST_EXECUTE: begin
                // FP16 encoding of lane index:
                // lane 0 = 0x0000 (0.0), lane 1 = 0x3C00 (1.0),
                // lane 2 = 0x4000 (2.0), ..., lane 15 = 0x4B80 (15.0)
                for (int lane = 0; lane < ROWS; lane++) begin
                    // Use SystemVerilog shortreal conversion
                    shortreal fval;
                    bit [31:0] fp32_bits;
                    bit [4:0] exp16;
                    bit [9:0] mant16;
                    ...
                    if (lane == 0) begin
                        core_activations[lane] = 16'h0000;
                    end else begin
                        exp16 = fp32_bits[30:23] - 8'd112;
                        mant16 = fp32_bits[22:13];
                        core_activations[lane] = {fp32_bits[31], exp16, mant16};
                    end
                end
                core_acc_valid = 1'b1;
                core_acc_clear = 1'b1;
