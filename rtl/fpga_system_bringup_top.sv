...
module fpga_system_bringup_top (
    ...
    // Drain Wires
    // =========================================================================
    (* mark_debug = "true" *) logic drain_re;
    ...
    initial begin
        ...
        // Each 128-bit word = 16 bytes, each byte = 0x01
        for (int row = 0; row < 16; row++)
            test_mem[row] = {16{8'h01}};

        // Activation rows at beat address 256..271 (byte 0x2000, beat = 0x2000/16)
        // Each byte[lane] = lane[7:0]
        ...
        if (!rst_n) begin
            ...
            if (!mem_rd_active) begin
                ...
                if (axi_arvalid && axi_arready) begin
                    mem_rd_active    <= 1'b1;
                    // Convert byte address to beat address (128b = 16B per beat)
    ...
    typedef enum logic [3:0] {
        ST_IDLE          = 4'd0,
        // DMA weight
        ST_CFG_WT        = 4'd1,   // Write DMA_CFG: target=weight, mode=preload
        ST_SRC_WT        = 4'd2,   // Write DMA_SRC_LO: weight base address
        ST_BURST_WT      = 4'd3,   // Write DMA_BURST: len=15, bursts=1
        ST_START_DMA_WT  = 4'd4,   // Write CTRL: start_dma
        ST_WAIT_DMA_WT   = 4'd5,   // Poll STATUS until dma_done
        // Swap + preload
        ST_SWAP_PRELOAD  = 4'd6,   // Write CTRL: swap_banks + start_npu (mode=preload)
        ST_WAIT_PRELOAD  = 4'd7,   // Poll STATUS until npu_done
        // DMA activation + execute
        ST_CFG_ACT       = 4'd8,   // Write DMA_CFG: target=act, mode=execute
        ST_SRC_ACT       = 4'd9,   // Write DMA_SRC_LO
        ST_START_DMA_ACT = 4'd10,  // Write CTRL: start_dma
        ST_WAIT_DMA_ACT  = 4'd11,  // Poll STATUS until dma_done
        // Swap + execute
        ST_SWAP_EXEC     = 4'd12,  // Write CTRL: swap_banks + start_npu
        ST_WAIT_EXEC     = 4'd13,  // Poll STATUS until npu_done
        // Drain & check
        ST_DRAIN         = 4'd14,
        ST_DONE          = 4'd15
    } selftest_state_t;
                    ...
                    case (drain_phase)
                        ...
                        2'd2: begin
                            ...
                            if (drain_row == 0) begin
                                result_match_latched <= (psum_drain_out[0] == EXPECTED_SUM) &&
                                                        (psum_drain_out[15] == EXPECTED_SUM);
                            end else begin
                                if ((psum_drain_out[0] != EXPECTED_SUM) ||
                                    (psum_drain_out[15] != EXPECTED_SUM))
                                    result_match_latched <= 1'b0;
                            end

                            if (drain_row == 5'd15) begin
                                drain_phase <= '0;
                                st_state    <= ST_DONE;
                            end else begin
