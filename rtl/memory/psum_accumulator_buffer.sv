...
module psum_accumulator_buffer #(
    ...
    // Port B: Mode-Dependent Vector Addition & Write-Back
    genvar i;
    generate
        for (i = 0; i < COLS; i++) begin : VEC_ADD
            if (DATA_MODE == NPU_DATA_MODE_INT8) begin : MODE_INT8
                logic signed [31:0] int32_acc_sum;
                assign int32_acc_sum = $signed(acc_rd_data[i]) + $signed(array_psum_q[i]);
                assign acc_write_data[i*DATA_WIDTH +: DATA_WIDTH] =
                    acc_clear_q ? array_psum_q[i] : int32_acc_sum;
            end else begin : MODE_FP16
                logic [31:0] fp32_acc_sum;
                fp32_adder u_acc_adder (
                    .fp32_a  (acc_rd_data[i]),
                    .fp32_b  (array_psum_q[i]),
                    .fp32_sum(fp32_acc_sum)
                );
                assign acc_write_data[i*DATA_WIDTH +: DATA_WIDTH] =
                    acc_clear_q ? array_psum_q[i] : fp32_acc_sum;
            end
        end
    endgenerate
